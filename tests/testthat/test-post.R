...
test_that("Post text data", {
  ...
  req <- curl_fetch_memory("http://test.opencpu.org/post", handle = h)
  ...
  txt <- readLines(curl("http://test.opencpu.org/post", handle = h))
  expect_equal(rawToChar(req$content), paste0(txt, "\n", collapse=""))

  # Using download interface
  tmp <- tempfile()
  curl_download("http://test.opencpu.org/post", tmp, handle = h)
  txt2 <- readLines(tmp)
  unlink(tmp)
