...
test_that("Post text data", {
  ...
  req <- curl_fetch_memory("http://eu.httpbin.org/post", handle = h)
  ...
  txt <- readLines(curl("http://eu.httpbin.org/post", handle = h))
  expect_equal(rawToChar(req$content), paste0(txt, "\n", collapse=""))

  # Using download interface
  tmp <- tempfile()
  curl_download("http://eu.httpbin.org/post", tmp, handle = h)
  txt2 <- readLines(tmp)
  unlink(tmp)
