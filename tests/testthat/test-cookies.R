...
test_that("Add some cookies", {
  req <- curl_fetch_memory("http://test.opencpu.org/cookies/set?foo=123&bar=ftw", handle = h)
  cookies <- handle_cookies(h);
  expect_is(cookies, "data.frame")
...
test_that("Delete a cookie", {
  req <- curl_fetch_memory("http://test.opencpu.org/cookies/delete?foo", handle = h)
  cookies <- handle_cookies(h)
...
test_that("Overwrite a cookie", {
  req <- curl_fetch_memory("http://test.opencpu.org/cookies/set?foo=888&bar=999", handle = h)
  cookies <- handle_cookies(h)
  foo <- subset(cookies, name == "foo")
  bar <- subset(cookies, name == "bar")
