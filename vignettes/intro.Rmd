...
The curl package implements three ways to retrieve data from a URL. The `curl_fetch_memory` function is a synchronous interface which returns a list with content of the server response.
...
After the handle has been configured, it can be used with any of the three download interfaces to issue the request. For example `curl_fetch_memory` will load store the output of the request in memory:
