...
Requesting a connection in read-write mode (`"r+"` or `"r+b"`) creates a bidirectional socket connection, similar to base `socketConnection()`. In this mode libcurl only establishes the connection (including the TLS handshake, certificate verification, and proxy traversal), without speaking the protocol, and you can exchange raw bytes on the socket with the usual reading and writing functions. This can be used to speak custom protocols over TLS, or protocol extensions that libcurl does not implement, such as the IMAP `IDLE` command.

Unlike regular curl connections, socket connections are not opened automatically: you call `open()` on the connection yourself. This way you can open them in non-blocking mode, as explained in the previous section, such that reads only return data that is available without waiting, and `isIncomplete()` tells whether the peer is still connected:
...
Alternatively, after a regular blocking `open(con)`, reads wait for data to arrive, which is convenient for an interactive request/response dialogue. Note that for pingpong protocols such as `imaps://` or `smtps://`, libcurl consumes the initial server greeting while connecting, so the first thing to read is the response to your own first command.
...
Some curl options take a long in C that actually corresponds to an ENUM value. 

For example the [CURLOPT_USE_SSL](https://curl.se/libcurl/c/CURLOPT_USE_SSL.html) docs explains that there are 4 possible values for this option:
...
Another example is the [CURLOPT_HTTP_VERSION](https://curl.se/libcurl/c/CURLOPT_HTTP_VERSION.html) option. This option is needed to disable or enable HTTP/2 or HTTP/3. However some users are not aware this is actually an ENUM and not a regular numeric value.
...
Note that a value `1` corresponds to HTTP 1.0 which is a legacy version of HTTP that you should not use. Code that sets `http_version` to `1` (or even `1.1` which R simply rounds to 1) is almost certainly a bug; we need to pick one of the `CURL_HTTP_VERSION*` values.
