...
Note that for pingpong protocols such as `imaps://` or `smtps://`, libcurl consumes the initial server greeting while connecting, so the first thing to read is the response to your own first command. Such a socket connection can also be opened in non-blocking mode to poll for data, for example to wait for incoming IMAP `IDLE` notifications.
