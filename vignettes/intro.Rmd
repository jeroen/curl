...
The R bindings will automatically do some type checking and coercion to convert R values to appropriate libcurl option values. For example these give errors:
 
```{r, error = TRUE}
# Error beause CURLOPT_MASFILESIZE must be a number
handle <- new_handle(maxfilesize = "foo")
```

Logical (boolean) values in R automatically get converted to `0` or `1` for example [CURLOPT_VERBOSE](https://curl.haxx.se/libcurl/c/CURLOPT_VERBOSE.html):
...
However R does not know if an option is actually boolean. So passing `TRUE`/ `FALSE` to any numeric option will simply set it to `0` or `1` without a warning or error.

### ENUM options (and HTTP/2)
...
Another example is the [CURLOPT_HTTP_VERSION](https://curl.haxx.se/libcurl/c/CURLOPT_HTTP_VERSION.html) option. This option is needed to disable or enable HTTP/2. However some users are not aware this is actually an ENUM! The [docs](https://curl.haxx.se/libcurl/c/CURLOPT_HTTP_VERSION.html) explain HTTP_VERSION can be set to one of several strategies for negotiating the HTTP version between client and server. Valid values are:
...
The default for http_version is `CURL_HTTP_VERSION_2TLS` which attempts HTTP/2 only for encrypted (HTTPS) connections. If you want to __disable http/2__ for whatever reason, you should set `CURL_HTTP_VERSION_1_1` which is `2`:

```{r}
# Force using HTTP 1.1
