...
```{r}
req <- curl_fetch_memory("https://hb.r-universe.dev/get?foo=123")
str(req)
...
```{r}
tmp <- tempfile()
curl_download("https://hb.r-universe.dev/get?bar=456", tmp)
...
curl_fetch_multi('https://hb.r-universe.dev/blabla', done = cb, pool = pool)
```
...
```{r}
req <- curl_fetch_memory("https://hb.r-universe.dev/post", handle = h)
jsonlite::prettify(rawToChar(req$content))
```
...
```{r}
con <- curl("https://hb.r-universe.dev/post", handle = h)
...
```{r}
tmp <- tempfile()
curl_download("https://hb.r-universe.dev/post", destfile = tmp, handle = h)
jsonlite::prettify(readLines(tmp))
```
...
```{r}
curl_fetch_multi("https://hb.r-universe.dev/post", handle = h, done = function(res){
...
req <- curl_fetch_memory("https://hb.r-universe.dev/cookies/set?baz=moooo", handle = h)
handle_cookies(h)
...
```{r}
req1 <- curl_fetch_memory("https://hb.r-universe.dev/get")
...
new_handle() %>%
  ...
  curl_fetch_memory(url = "https://hb.r-universe.dev/post") %$% content %>% 
