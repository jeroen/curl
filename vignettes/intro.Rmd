...
```{r}
req <- curl_perform("http://httpbin.org/get")
...
```{r}
tmp <- tempfile()
curl_download("http://httpbin.org/get", tmp)
...
As the example how to use `readLines` on the opened connection object to read `n` lines at a time. Similarly `readBin` is used to read `n` bytes at a time for stream parsing binary data.
...
```{r}
req <- curl_perform("http://httpbin.org/status/418")
print(req$status_code)
```
