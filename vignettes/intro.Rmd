...
As of libcurl 7.62.0, the default for http_version is `CURL_HTTP_VERSION_2TLS` which attempts HTTP/2 only for encrypted (HTTPS) connections. If you want to enable http/2 on older versions of libcurl you need to set it manually:

```{r}
# Use HTTP/2 when possbile
if(curl::curl_version()$http2){
  handle_setopt(handle, http_version = 4)
}
```

Note that this can only be done if HTTP/2 support has been enabled in libcurl on your platform:

```{r}
curl::curl_version()['http2']
```

If you want to explicitly __disable http/2__ for whatever reason, you should set `CURL_HTTP_VERSION_1_1` which is `2`:
