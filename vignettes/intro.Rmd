...
Note that for pingpong protocols such as `imaps://` or `smtps://`, libcurl consumes the initial server greeting while connecting, so the first thing to read is the response to your own first command. Like base `socketConnection()`, blocking reads wait at most `getOption("timeout")` seconds (default 60) for data, and then return whatever is available; use `isIncomplete()` to distinguish a timeout from the peer having closed the connection. A socket connection can also be opened in non-blocking mode to poll for data without waiting, for example for incoming IMAP `IDLE` notifications.
